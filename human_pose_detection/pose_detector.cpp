...
namespace human_pose_estimation {
	...
	const size_t pose_detector::keypointsNumber = 18;
	// Constructor
	pose_detector::pose_detector(const std::string& modelPath,
		const std::string& targetDeviceName_,
		bool enablePerformanceReport) : minJointsNumber(3),
		stride(8),
		pad(cv::Vec4i::all(0)),
	...
	void pose_detector::initialize() {

	}

	void pose_detector::finalize() {

	}

	
	std::vector<HumanPose> pose_detector::estimate(const cv::Mat& image) {
	...
	void pose_detector::preprocess(const cv::Mat& image, uint8_t* buffer) const {
		cv::Mat resizedImage;
	...
	std::vector<HumanPose> pose_detector::postprocess(
	...
	std::vector<HumanPose> pose_detector::extractPoses(
		const std::vector<cv::Mat>& heatMaps,
	...
	void pose_detector::resizeFeatureMaps(std::vector<cv::Mat>& featureMaps) const {
	...
	void pose_detector::correctCoordinates(std::vector<HumanPose>& poses,
	...
	bool pose_detector::inputWidthIsChanged(const cv::Size& imageSize) {

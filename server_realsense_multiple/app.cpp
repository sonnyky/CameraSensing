...
void Server::osc_send(string str) {
	...
	char *toSendText = new char[str.size() + 1];
	std::strcpy(toSendText, str.c_str());
	transmitSocket.Send(str.data(), elem.ByteSize());
}
...
void Server::receive_data_length() {
		...
		if (iResult_ > 0) {
			...
			printf("[receive_data_length] iResult recv failed with error: %d\n", WSAGetLastError());
			return;
		}
}
...
void Server::parse_data(string test) {
	...
	cout << "device number " << last_elem.device_number(0) << endl;
}

int Server::receive_data_chunk(string& test) {

	vector<char> recv_char(messagelength_);
	iDataResult_ = recv(ClientSocket_, recv_char.data(), recv_char.size(), 0);
	...
	if (iDataResult_ > 0) {
		...
		printf("[receive_data_chunk] iDataResult recv failed with error: %d\n", WSAGetLastError());
		return 1;
	}
	return 0;
}
...
//	if (last_elem.x_size() == last_elem.y_size() && last_elem.x_size() >= 4) {
		...
		circle(merged, Point(2560, 720), 20.0, Scalar(0, 0, 255), -1, 8);
		...
		for (int i = 0; i < last_elem.x_size(); i++ ) {
			circle(merged, Point(last_elem.x(i), last_elem.y(i)), 20.0, Scalar(255, 0, 0), 2, 8);
		}
